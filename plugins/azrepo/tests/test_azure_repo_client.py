...
class TestAzureRepoClientProperties:
    ...
    def test_description_property(self, azure_repo_client):
        ...
        assert azure_repo_client.description == "Interact with Azure DevOps repositories and pull requests"
...
class TestAzureRepoClientCommands:
    ...
    @pytest.mark.asyncio
    async def test_run_az_command_success(self, azure_repo_client, mock_command_success_response):
        """Test successful Azure CLI command execution."""
        # Mock the executor methods
        azure_repo_client.executor.execute_async = AsyncMock(return_value={"token": "test_token"})
        azure_repo_client.executor.query_process = AsyncMock(return_value={
            "success": True,
        ...
        azure_repo_client.executor.execute_async.assert_called_once_with("az repos pr list --output json", None)
...
class TestListPullRequests:
    ...
    @pytest.mark.asyncio
    async def test_list_pull_requests_basic(self, azure_repo_client, mock_pr_list_response):
        """Test basic pull request listing."""
        azure_repo_client._run_az_command = AsyncMock(return_value=mock_pr_list_response)
    ...
    @pytest.mark.asyncio
    async def test_list_pull_requests_with_filters(self, azure_repo_client, mock_pr_list_response):
...
class TestGetPullRequest:
    ...
    @pytest.mark.asyncio
    async def test_get_pull_request_basic(self, azure_repo_client, mock_command_success_response):
        """Test getting a specific pull request."""
        azure_repo_client._run_az_command = AsyncMock(return_value=mock_command_success_response)

        result = await azure_repo_client.get_pull_request(123)

        azure_repo_client._run_az_command.assert_called_once_with("repos pr show --id 123")
        assert result == mock_command_success_response

    @pytest.mark.asyncio
    async def test_get_pull_request_with_org(self, azure_repo_client, mock_command_success_response):
        """Test getting a pull request with organization."""
        azure_repo_client._run_az_command = AsyncMock(return_value=mock_command_success_response)

        result = await azure_repo_client.get_pull_request(123, organization="https://dev.azure.com/myorg")
...
class TestCreatePullRequest:
    ...
    @pytest.mark.asyncio
    async def test_create_pull_request_basic(self, azure_repo_client, mock_command_success_response):
        ...
        expected_command = 'repos pr create --title "Test PR" --source-branch feature/test'
        azure_repo_client._run_az_command.assert_called_once_with(expected_command)
        assert result == mock_command_success_response

    @pytest.mark.asyncio
    async def test_create_pull_request_full_options(self, azure_repo_client, mock_command_success_response):
...
class TestUpdatePullRequest:
    ...
    @pytest.mark.asyncio
    async def test_update_pull_request_basic(self, azure_repo_client, mock_command_success_response):
    ...
    @pytest.mark.asyncio
    async def test_update_pull_request_with_flags(self, azure_repo_client, mock_command_success_response):
        ...
        expected_command = 'repos pr update --id 123 --auto-complete true --squash false --draft true'
        azure_repo_client._run_az_command.assert_called_once_with(expected_command)
        assert result == mock_command_success_response
...
class TestVotingAndReviewers:
    ...
    @pytest.mark.asyncio
    async def test_add_reviewers(self, azure_repo_client, mock_command_success_response):
        """Test adding reviewers to a pull request."""
        azure_repo_client._run_az_command = AsyncMock(return_value=mock_command_success_response)

        result = await azure_repo_client.add_reviewers(123, ["user1", "user2"])

        expected_command = "repos pr reviewer add --id 123 --reviewers user1 --reviewers user2"
        azure_repo_client._run_az_command.assert_called_once_with(expected_command)
        assert result == mock_command_success_response

    @pytest.mark.asyncio
    async def test_add_work_items(self, azure_repo_client, mock_command_success_response):
        """Test adding work items to a pull request."""
        azure_repo_client._run_az_command = AsyncMock(return_value=mock_command_success_response)

        result = await azure_repo_client.add_work_items(123, [456, 789])

        expected_command = "repos pr work-item add --id 123 --work-items 456 --work-items 789"
        azure_repo_client._run_az_command.assert_called_once_with(expected_command)
        assert result == mock_command_success_response
...
class TestExecuteTool:
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_list_pull_requests(self, azure_repo_client, mock_pr_list_response):
        """Test execute_tool with list_pull_requests operation."""
        azure_repo_client.list_pull_requests = AsyncMock(return_value=mock_pr_list_response)
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_get_pull_request(self, azure_repo_client, mock_command_success_response):
        """Test execute_tool with get_pull_request operation."""
        azure_repo_client.get_pull_request = AsyncMock(return_value=mock_command_success_response)
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_create_pull_request(self, azure_repo_client, mock_command_success_response):
        """Test execute_tool with create_pull_request operation."""
        azure_repo_client.create_pull_request = AsyncMock(return_value=mock_command_success_response)

...
class TestWorkItems:
    ...
    async def test_get_work_item_with_date_formats(
        ...
        azure_repo_client._run_az_command.assert_called_with(expected_command)

        # Test with date and time
        azure_repo_client._run_az_command.reset_mock()
        result = await azure_repo_client.get_work_item(
            work_item_id=123,
    ...
    @pytest.mark.asyncio
    async def test_get_work_item_error_handling(self, azure_repo_client):
        """Test work item retrieval error handling."""
        error_response = {
            "success": False,
            "error": "Work item not found"
        }
        azure_repo_client._run_az_command = AsyncMock(return_value=error_response)

        result = await azure_repo_client.get_work_item(999)

...
class TestKnowledgeQueryTool:
    ...
    @pytest.mark.asyncio
    @patch('plugins.knowledge_indexer.tool.SentenceTransformer')
    @patch('plugins.knowledge_indexer.tool.ChromaVectorStore')
    ...
    async def test_execute_tool_exception_handling(self):
        ...
        with patch('plugins.knowledge_indexer.tool.SentenceTransformer', side_effect=Exception("Test error")):
            result = await tool.execute_tool({
                "query": "test query",
                "collection": "test_collection"
            })

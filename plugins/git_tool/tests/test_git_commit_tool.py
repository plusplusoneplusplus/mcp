...
class TestGitCommitTool:
    ...
    async def test_invalid_operation(self, git_commit_tool):
        ...
        result = await git_commit_tool.execute_tool({"operation": "invalid_op", "repo_path": "/test"})
    ...
    async def test_commit_success(self, mock_repo_class, git_commit_tool, mock_repo):
        ...
        assert "Changes committed successfully with hash abc123def456" in result["result"]
    ...
    async def test_commit_git_error(self, mock_repo_class, git_commit_tool, mock_repo):
        ...
        mock_repo_class.return_value = mock_repo
        mock_repo.index.commit.side_effect = GitCommandError("commit", 1, "nothing to commit")
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_success(self, mock_repo_class, git_commit_tool, mock_repo):
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_with_custom_remote(self, mock_repo_class, git_commit_tool, mock_repo):
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_conflict(self, mock_repo_class, git_commit_tool, mock_repo):
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_no_remote(self, mock_repo_class, git_commit_tool, mock_repo):
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_no_tracking(self, mock_repo_class, git_commit_tool, mock_repo):
    ...
    @pytest.mark.asyncio
    @patch('git.Repo')
    async def test_pull_rebase_generic_error(self, mock_repo_class, git_commit_tool, mock_repo):

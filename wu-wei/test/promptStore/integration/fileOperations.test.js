...
const commands_1 = require("../../../src/promptStore/commands");
...
suite('File Operations Integration Tests', () => {
    ...
    suiteSetup(async () => {
        ...
        fileOperationCommands = new commands_1.FileOperationCommands(fileOperationManager, templateManager);
    ...
    test('Prompt duplication workflow', async () => {
        ...
        try {
            ...
            const duplicateExists = await fs.access(path.join(testDir, 'test', 'duplicated-prompt.md')).then(() => true, () => false);
    ...
    test('Error handling for invalid operations', async () => {
        ...
        vscode.window.showWarningMessage = async () => 'Delete';
        try {
            await vscode.commands.executeCommand('wu-wei.promptStore.deletePrompt', path.join(testDir, 'non-existent.md'));
    ...
    test('Export functionality integration', async () => {
        ...
        const originalShowQuickPick = vscode.window.showQuickPick;
        vscode.window.showQuickPick = async () => ({ label: 'JSON', value: 'json' });

...
@pytest.mark.parametrize(
    "code_enabled,yaml_enabled,expected_tools",
    [
...
def test_end_to_end_tool_filtering(
    clean_environment, code_enabled, yaml_enabled, expected_tools
):
    ...
    for tool_name in expected_tools:
        assert (
            tool_name in filtered_instances
        ), f"Tool {tool_name} should be in filtered instances"


@pytest.mark.parametrize(
    "tool_name,source,should_be_enabled",
    [
...
def test_server_tool_call_filtering(
    clean_environment, tool_name, source, should_be_enabled
):
    """Test that server tool calling respects the source configuration."""
    # Configure sources
    os.environ["MCP_REGISTER_CODE_TOOLS"] = (
        "1" if source == "code" and should_be_enabled else "0"
    )
    os.environ["MCP_REGISTER_YAML_TOOLS"] = (
        "1" if source == "yaml" and should_be_enabled else "0"
    )

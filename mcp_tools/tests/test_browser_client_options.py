...
class TestBrowserClientOptions:
    ...
    def test_edge_browser_options(self, mock_socket, mock_service, mock_edge):
        ...
        with patch.object(
            SeleniumBrowserClient,
            "_setup_edge_driver",
            return_value="/path/to/msedgedriver",
        ):
            # Create a client
    ...
    @patch("mcp_tools.browser.selenium_client.ChromeService")
    @patch("socket.socket")
    def test_custom_headless_option_respected(
        self, mock_socket, mock_service, mock_chrome
    ):
        ...
        headless_count = sum(
            1 for arg in options.arguments if arg.startswith("--headless")
        )

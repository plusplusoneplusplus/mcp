...
def test_get_time_now_with_delta():
    ...
    dt = datetime.datetime.strptime(result, "%Y-%m-%d %H:%M:%S").replace(
...
async def test_time_tool_get_time_with_delta():
    ...
    result = await tool.execute_tool(
        {"operation": "get_time", "time_point": base, "delta": "5m", "timezone": "UTC"}
    )
    assert result == "2024-07-01 12:05:00"
...
async def test_time_tool_get_time_now_with_delta():
    ...
    now = datetime.datetime.now(datetime.timezone.utc)
    result = await tool.execute_tool(
        {"operation": "get_time", "time_point": "now", "delta": "1m", "timezone": "UTC"}
    )
    dt = datetime.datetime.strptime(result, "%Y-%m-%d %H:%M:%S").replace(
        tzinfo=datetime.timezone.utc
    )
...
async def test_time_tool_get_time_invalid_delta():
    ...
    res = await tool.execute_tool(
        {"operation": "get_time", "time_point": "2024-07-01 12:00:00", "delta": "bad"}
    )

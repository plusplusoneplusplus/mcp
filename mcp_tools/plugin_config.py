...
class PluginConfig:
    ...
    def _load_from_env(self):
        ...
        if env_excluded:
            additional_excluded = {cls.strip() for cls in env_excluded.split(",") if cls.strip()}
        ...
        if env_excluded_tools:
            excluded_tools = {tool.strip() for tool in env_excluded_tools.split(",") if tool.strip()}
    ...
    def should_register_tool_class(self, class_name: str, tool_name: str, yaml_tools: Set[str]) -> bool:
        ...
        if self.yaml_overrides_code and tool_name in yaml_tools and not class_name.startswith("YamlTool_"):
            logger.info(f"Skipping registration of {class_name} (tool: {tool_name}) because a YAML definition exists")
            return False

...
class TestImageToolArgumentValidation:
    ...
    def test_handle_tool_valid_arguments(self, mock_get_session_image):
        ...
        mock_result = [
            ImageContent(type="image", data="fake_data", mimeType="image/png")
        ]
...
class TestGetSessionImageFunction:
    ...
    def test_get_session_image_success_png(
        self, mock_guess_type, mock_file, mock_is_file, mock_get_dir
    ):
    ...
    @patch("mimetypes.guess_type")
    def test_get_session_image_success_jpeg(
        self, mock_guess_type, mock_file, mock_is_file, mock_get_dir
    ):
    ...
    def test_get_session_image_success_json(
        self, mock_file, mock_is_file, mock_get_dir
    ):
        """Test successful JSON retrieval returns TextContent with raw text."""
        mock_image_dir = Path("/test/images")
        mock_get_dir.return_value = mock_image_dir
        mock_is_file.return_value = True

        result = image_tool.get_session_image("test_session", "data.json")
    ...
    @patch("server.image_tool.get_image_dir")
    @patch("pathlib.Path.is_file")
    @patch("builtins.open", new_callable=mock_open, read_data='{"upper":true}')
    def test_get_session_image_success_json_uppercase_ext(
        self, mock_file, mock_is_file, mock_get_dir
    ):
        """Test JSON retrieval works with uppercase extension."""
        mock_image_dir = Path("/test/images")
        mock_get_dir.return_value = mock_image_dir
        mock_is_file.return_value = True

        result = image_tool.get_session_image("test_session", "config.JSON")

        assert isinstance(result[0], TextContent)
        assert result[0].text == '{"upper":true}'

    @patch("server.image_tool.get_image_dir")
    @patch("pathlib.Path.is_file")
    @patch("builtins.open", new_callable=mock_open, read_data=b"plain")
    @patch("mimetypes.guess_type")
    def test_get_session_image_non_json_not_text(
        self, mock_guess_type, mock_file, mock_is_file, mock_get_dir
    ):
        """Ensure only JSON is treated as text; others remain binary/image response."""
        mock_image_dir = Path("/test/images")
        mock_get_dir.return_value = mock_image_dir
        mock_is_file.return_value = True
        mock_guess_type.return_value = (None, None)

        result = image_tool.get_session_image("test_session", "notes.txt")
    ...
    @patch("mimetypes.guess_type")
    def test_get_session_image_unknown_mime_type(
        self, mock_guess_type, mock_file, mock_is_file, mock_get_dir
    ):
    ...
    def test_get_session_image_permission_error(
        self, mock_file, mock_is_file, mock_get_dir
    ):
    ...
    @patch("mimetypes.guess_type")
    def test_get_session_image_empty_file(
        self, mock_guess_type, mock_file, mock_is_file, mock_get_dir
    ):
    ...
    def test_get_session_image_path_construction(self):
        ...
        with (
            patch("server.image_tool.get_image_dir") as mock_get_dir,
...
class TestMimeTypeDetection:
    ...
    def test_mime_type_detection_various_formats(
        self, mock_file, mock_is_file, mock_get_dir
    ):
...
class TestBase64Encoding:
    ...
    @patch("pathlib.Path.is_file")
    @patch("mimetypes.guess_type")
    def test_base64_encoding_various_data(
        self, mock_guess_type, mock_is_file, mock_get_dir
    ):
...
class TestServerIntegration:
    ...
    @pytest.mark.skipif(
        os.name == "nt",
        reason="Skipping on Windows due to timeout issues with MCP client session cleanup (Issue #205)",
    )
    ...
    async def test_builtin_tool_error_handling_via_server(self, mcp_client_info):
        ...
        async with create_mcp_client(server_url, worker_id) as session:
            ...
            text_val = getattr(content, "text", "") or ""
            assert "required" in text_val.lower()
...
class TestResponseFormat:
    ...
    def test_successful_response_format(
        ...
        assert (
            ...
            or content.mimeType == "application/octet-stream"
        )

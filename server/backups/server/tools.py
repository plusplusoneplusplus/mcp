...
class ListTasksTool(ToolExecutor):
    ...
    async def execute(self, arguments: dict) -> list[TextContent]:
        ...
        if not tasks:
            return [TextContent(type="text", text="No predefined tasks found.")]
        ...
        for name, task in tasks.items():
            ...
            timeout = (
                f", timeout: {task.get('timeout')} seconds" if "timeout" in task else ""
            )
            ...
            if "commands" in task:
                ...
                all_commands = "\n  ".join(
                    [f"{os}: {cmd}" for os, cmd in task["commands"].items()]
                )
...
class ListInstructionsTool(ToolExecutor):
    ...
    async def execute(self, arguments: dict) -> list[TextContent]:
        ...
        instruction_list = "\n".join(
            [f"- {p.name}: {p.description}" for p in available_instructions]
        )

        return [
            TextContent(
                type="text", text=f"Available instructions:\n{instruction_list}"
            )
        ]
...
class GetInstructionTool(ToolExecutor):
    ...
    async def execute(self, arguments: dict) -> list[TextContent]:
        ...
        template = (
            f"\nTemplate:\n{instruction_data.get('template', 'No template available')}"
            if "template" in instruction_data
            else ""
        )

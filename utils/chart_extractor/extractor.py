...
def _get_adaptive_chart_boundaries(
    ...
    left_side_text = [region for region in text_regions 
                     if region[0] < chart_x and 
                        region[0] + region[2] >= chart_x - expansion_zone and
                        region[1] + region[3] > chart_y and 
                        region[1] < chart_y + chart_h]
    
    right_side_text = [region for region in text_regions 
                      if region[0] > chart_x + chart_w and 
                         region[0] <= chart_x + chart_w + expansion_zone and
                         region[1] + region[3] > chart_y and 
                         region[1] < chart_y + chart_h]
    ...
    for tx, ty, tw, th in text_regions:
        ...
        if has_multiple_y_axes and tx > chart_x + chart_w + expansion_zone * 0.7:
            if debug:
                debug_info['skipped_text'].append((tx, ty, tw, th, 'outside_right_y_axis_range'))
            continue
        ...
        if (tx + tw >= chart_x - expansion_zone and tx <= chart_x + chart_w + expansion_zone and
            ty + th >= chart_y - expansion_zone and ty <= chart_y + chart_h + expansion_zone):
            
            # Text categorization with stricter criteria
            
            # Title: Above the chart, horizontally centered
            is_title = (abs(text_center_x - chart_center_x) < chart_w * 0.4 and  # Centered with 40% tolerance
                       ty < chart_y and ty + th >= chart_y - expansion_zone * 0.8)  # Closer to chart top
            
            # X-axis label: Below the chart, horizontally aligned
            is_x_axis_label = (ty >= chart_y + chart_h - 10 and 
                              ty <= chart_y + chart_h + expansion_zone * 0.6 and  # Reduced vertical range 
                              tx + tw > chart_x and tx < chart_x + chart_w)  # Must overlap horizontally
            
            # Y-axis label: Left of the chart, vertically aligned
            is_y_axis_label = (tx < chart_x and 
                              tx + tw >= chart_x - expansion_zone * 0.7 and  # Reduced horizontal range
                              ty + th > chart_y and ty < chart_y + chart_h)  # Must overlap vertically
                              
            # Y-axis secondary label: Right of the chart, vertically aligned
            is_y_axis_secondary = (tx > chart_x + chart_w - 5 and 
                                  tx <= chart_x + chart_w + expansion_zone * 0.7 and  # Reduced horizontal range
                                  ty + th > chart_y and ty < chart_y + chart_h)  # Must overlap vertically
                                  
            # Legend: Below or near the chart, not overlapping with axis labels
            is_legend = (not is_title and not is_x_axis_label and 
                        not is_y_axis_label and not is_y_axis_secondary and
                        tx + tw >= chart_x and 
                        tx <= chart_x + chart_w and
                        ty >= chart_y and 
                        ty <= chart_y + chart_h + expansion_zone * 0.5)
            
            # If this text appears to be related to the chart, include it
            if is_title or is_x_axis_label or is_y_axis_label or is_y_axis_secondary or is_legend:
                text_type = "title" if is_title else \
                           "x-axis" if is_x_axis_label else \
                           "y-axis" if is_y_axis_label else \
                           "y-axis2" if is_y_axis_secondary else \
                           "legend"
            ...
            debug_info['skipped_text'].append((tx, ty, tw, th, 'outside_expansion_zone'))
    ...
    if not relevant_text:
        ...
        if x_max - x_min > chart_w * 1.8:  # Reduced from 2.0 to 1.8
            ...
            if right_expansion > chart_w * 0.6:  # Reduced from 0.8 to 0.6
                x_max = min(x_max, chart_x + chart_w + chart_w * 0.4)  # Reduced from 0.5 to 0.4
                if debug:
...
def _merge_overlapping_boxes(boxes: List[Tuple[int, int, int, int]], overlap_threshold: float = 0.3) -> List[Tuple[int, int, int, int]]:
...
def _boxes_overlap(box1: Tuple[int, int, int, int], box2: Tuple[int, int, int, int], threshold: float) -> bool:
...
def _merge_boxes(box1: Tuple[int, int, int, int], box2: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:

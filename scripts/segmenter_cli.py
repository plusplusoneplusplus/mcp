...
async def main():
    ...
    if (
        args.persist_directory == "DEFAULT_PATH"
    ):  # Special value to use default if user wants persistence
    ...
    try:
        ...
        if args.operation == "segment":
            ...
            print(
                f"Segmenting and storing markdown file: {args.input_file} into collection '{args.collection_name}'..."
            )
            total_stored, segment_ids_by_type = segmenter.segment_and_store(
                markdown_content
            )
            ...
            print(
                f"Searching collection '{args.collection_name}' with query: '{args.query}'..."
            )

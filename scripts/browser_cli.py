...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test web pages using a browser client"
    )
    parser.add_argument("url", help="URL to visit")
    parser.add_argument(
        "--operation",
        "-o",
        choices=["html", "screenshot", "markdown"],
        default="html",
    ...
    parser.add_argument(
        "--browser",
        "-b",
        choices=["chrome", "edge", "chromium", "firefox", "webkit"],
        default="edge",
    ...
    try:
        ...
        browser_client = BrowserClientFactory.create_client(
            ...
            browser_type=(
                "edge"
                if args.client_type == "playwright" and args.browser == "edge"
                else args.browser
            ),
        )
                    ...
                    if markdown_result.get("extraction_success"):
                        ...
                        if not output_path:
                            ...
                            base_name = os.path.splitext(
                                os.path.basename(args.url.rstrip("/"))
                            )[0]
                            if (
                                not base_name or base_name == "index"
                            ):  # handle cases like example.com/ or example.com/index.html
                                base_name = "page"
                        ...
                        print(
                            f"Failed to extract markdown from HTML. Error: {markdown_result.get('error')}"
                        )
                else:
                    print(
                        f"Failed to retrieve HTML from {args.url} to generate markdown."
                    )

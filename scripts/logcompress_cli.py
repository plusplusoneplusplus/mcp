...
def compress_logs(
    ...
    if not parser_cls:
        raise ValueError(
            f"Unknown algorithm: {algorithm}. Available algorithms: {', '.join(ALGORITHMS.keys())}"
        )
    ...
    try:
        ...
        if len(df_structured) != len(logs_with_ts):
            click.echo(
                f"Warning: Number of parsed logs ({len(df_structured)}) does not match input ({len(logs_with_ts)})"
            )
        ...
        for i, (timestamp, _) in enumerate(logs_with_ts):
            ...
            result = {"t": template_id, "ts": timestamp, "p": params}
